#!/bin/bash
# Tears down the Invoice Ninja stack completely.
...
# Note: DB_PASSWORD, DB_ROOT_PASSWORD, IN_PASSWORD, and APP_KEY in .env are
#       not reset — replace them with GENERATE_ME beforehand for a fully fresh
#       install so start.sh regenerates them on the next run.
set -e
